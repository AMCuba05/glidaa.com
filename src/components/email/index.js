...
export default function Email() {
  ...
  const handleDelete = async () => {
    ...
    await Storage.remove(selectedTempalte.id)
      .then((result) => {
        setFileLink(null);
        setshowProgress(true);
        API.graphql(
          graphqlOperation(updateEmailTemplate, {
            input: { id: selectedTempalte.id, files: null },
          })
        ).then((data) => {
          setshowProgress(false);
        });
    ...
    if (!selectedTempalte?.id) {
      try {
        let res = await API.graphql(
          graphqlOperation(createEmailTemplate, { input: payload })
        );
      ...
      try {
        let res = await API.graphql(
          graphqlOperation(updateEmailTemplate, { input: payload })
        );
        let template = res.data.updateEmailTemplate;

        let newEmailTemalates = emailTemalates.filter(
          (x) => x.id !== template.id
        );
              ...
              <Col xs={12} lg="6">
                <Form onSubmit={submitHandler}>
                  ...
                  <Form.Group>
                    ...
                    <br></br>
                    {selectedTempalte.files && fileLink && (
                      <div>
                        Uploaded file{" "}
                        <a href={fileLink} target="blank">
                          {JSON.parse(selectedTempalte.files)[0].fileName}
                        </a>{" "}
                        &nbsp;{" "}
