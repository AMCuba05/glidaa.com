...
const orthoCameraMemory = {
	left: {
		position: new AFRAME.THREE.Vector3(-10, 0, 0),
		rotation: new AFRAME.THREE.Euler(),
	},
	right: {
		position: new AFRAME.THREE.Vector3(10, 0, 0),
		rotation: new AFRAME.THREE.Euler(),
	...
	top: {
		position: new AFRAME.THREE.Vector3(0, 10, 0),
		rotation: new AFRAME.THREE.Euler(),
	},
	bottom: {
		position: new AFRAME.THREE.Vector3(0, -10, 0),
	...
	back: {
		position: new AFRAME.THREE.Vector3(0, 0, -10),
		rotation: new AFRAME.THREE.Euler(),
	},
	front: {
		position: new AFRAME.THREE.Vector3(0, 0, 10),
...
class CameraTools {
	...
	init(inspector) {
		...
		inspector.currentCameraEl = sceneEl.camera.el;
		const originalCamera = sceneEl.camera.el;
		...
		EventTools.on('cameraperspectivetoggle', () => {
			this.saveOrthoCamera(inspector.camera , currentOrthoDir);
		...
		EventTools.on('cameraorthographictoggle', (dir) => {
			this.saveOrthoCamera(inspector.camera , currentOrthoDir);
	...
	setOrthoCamera = (camera, dir, ratio) => {
		const info = orthoCameraMemory[dir];
