...
class ViewportTools {
	 ...
	 * @memberof ViewportTools
	 */
	initTransformControls(inspector) {
		const sceneHelpers = inspector.sceneHelpers;
		const camera = inspector.camera;
		const transformControls = new AFRAME.THREE.TransformControls(camera, inspector.container);
	...
	initEvents(inspector) {
		...
		EventTools.on('inspectortoggle', active => {
			if (active) {
				...
				AFRAME.INSPECTOR.sceneEl.camera = inspector.cameras.original.getObject3D('camera');
				Array.prototype.slice.call(document.querySelectorAll('.a-enter-vr,.rs-base')).forEach(element => {
	 ...
	 * @memberof ViewportTools
	 */
	updateHelpers(inspector, object) {
		if (object) {
