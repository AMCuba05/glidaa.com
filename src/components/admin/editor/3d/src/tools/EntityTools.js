...
export const createEntity = (primitive, callback) => {
	...
	if (fragment && fragment.entity) {
		...
		const firstChild = createdFragment.firstChild;
...
export const updateEntity = (entity, propertyName, value) => {
	let splitName;
...
export const cloneSelectedEntity = () => {
	if (AFRAME.INSPECTOR.selectedEntity && AFRAME.INSPECTOR.selectedEntity.tagName.toLowerCase() !== 'a-scene') {
...
export const cloneEntity = (entity, deep) => {
	entity.flushToDOM();
	const clonedEntity = entity.cloneNode(deep);
...
export const findClosestEntity = (entity) => {
 ...
 */
export const buildEntities = (scene) => {
	const treeNodes = [
		{
			key: scene.id,
	...
	const traverseBuildTreeNode = (treeNode) => {
		for (let i = 0; i < treeNode.entity.children.length; i++) {
			const en = treeNode.entity.children[i] ;
			if (isInspector(en)) {
				continue;
...
export const isInspector = (en) => {
	return (
		'isInpsector' in en.dataset ||
 ...
 */
export function getEntityClipboardRepresentation(entity, ar) {
	const clone = prepareForSerialization(entity, ar) ;
	if (ar) {
		clone.setAttribute(
			'arjs',
			'sourceType: webcam; debugUIEnabled: true; trackingMethod: best; patternRatio: 0.75;',
		);
 ...
 */
const prepareForSerialization = (entity, ar) => {
	const clone = entity.cloneNode(false) ;
	...
	for (let i = 0, l = children.length; i < l; i++) {
		...
		if (
			...
			clone.appendChild(prepareForSerialization(child ));
		}
	}
 ...
 */
const optimizeComponents = (copy, source) => {
	...
	Object.keys(components).forEach(name => {
		...
		const optimalUpdate = getOptimalUpdate(component, implicitValue, currentValue);
 ...
 */
const stringifyComponentValue = (schema, data) => {
	const single = () => {
 ...
 */
const getImplicitValue = (component, source) => {
 ...
 *                                  `undefined`, otherwise.
 */
const getFromAttribute = (component, propertyName, source) => {
	let value;
	const mappings = source.mappings || {};
	const route = component.name + '.' + propertyName;
	const findAttribute = (mappings, route) => {
 ...
 */
const getMixedValue = (component, propertyName, source) => {
	let value;
 ...
 */
const getInjectedValue = (component, propertyName, source) => {
	let value;
 ...
 *                                   default.
 */
const getDefaultValue = (component, propertyName, source) => {
	if (!propertyName) {
 ...
 *                              the reference value.
 */
const getOptimalUpdate = (component, implicit, reference) => {
	...
	Object.keys(reference).forEach(key => {
		...
		// if (needsUpdate) {
		if (reference[key] && (key === 'src' || key === 'mtl' || key === 'obj')) {
...
export const exportToGLTF = (entity) => {
...
const filterHelpers = (object3D, visible) => {
	object3D.traverse(o => {
