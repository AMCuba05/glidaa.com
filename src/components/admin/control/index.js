...
export default function Index() {
  ...
  const updateMousePosition = async (ev) => {
    ...
    var payload = {
      id: "admin.admin",
      cursorPosition: `${window.scrollX},${window.scrollY},${window.innerWidth},${window.innerHeight},${email}`,
      phone: "+1",
    };
    try {
      let { data } = await API.graphql(
        graphqlOperation(updateUserActivity, { input: payload })
      );
    } catch (ex) {
      let { _data } = await API.graphql(
        graphqlOperation(createUserActivity, { input: payload })
      );
    }
  };
  ...
  useEffect(() => {
    ...
    const formSend = async () => {
      ...
      try {
        let { data } = await API.graphql(
          graphqlOperation(getUserActivity, { id: email })
        );
      ...
      setStillLoading();
      let res = await fetch(
        `https://i6smufsvj6.execute-api.us-east-1.amazonaws.com/live/visit?email=${email}&existingUser=true`
      );
      setLoadingComplete();
      res.json().then((data) => {
        let userData;
  ...
  return (
    <div>
      <div className="admin-card-container">
        <h5 style={{ margin: "50px" }}>{loading}</h5>
        {!user && (
          <Card
            bg={"danger"}
            text={"white"}
          ...
          >
            ...
            <Card.Body>
              ...
              <Card.Text>No user found with email: {email}</Card.Text>
              <Card
                bg={"secondary"}
                text={"white"}
              ...
              >
                ...
                <Card.Body>
                  ...
                  <Card.Text>
                    <Button
                      variant={!isScrollControlled ? "danger" : "warning"}
                      onClick={toggleScrollControl}
                    >
                      {!isScrollControlled ? "Take" : "Release"} control
                    </Button>
                    <br></br>
                    scroll(X, Y) :{" "}
                    {userActivityDetails?.cursorPosition?.split(",")[0]},{" "}
                    {userActivityDetails?.cursorPosition?.split(",")[1]}{" "}
                    <br></br>
                    window(W, H) :{" "}
                    {userActivityDetails?.cursorPosition?.split(",")[2]},{" "}
                    {userActivityDetails?.cursorPosition?.split(",")[3]}
                  </Card.Text>
                </Card.Body>
                {userActivityDetails?.cursorPosition?.split(",")[2] !==
                  window.innerWidth &&
                  userActivityDetails?.cursorPosition?.split(",")[3] !==
                    ...
                    <Card.Footer className="danger-footer">
                      Warning: Your window's WIDTH and HEIGHT are (
                      {windowState.innerWidth}, {windowState.innerHeight}) which
                      are not same as WIDTH and HEIGHT of visitor's window.
                    </Card.Footer>
                  )}
              </Card>
            </Card.Body>
        ...
        {!loading && user && (
          <Card style={{ width: "70%", margin: "50px" }}>
            <Card.Header>User details</Card.Header>
            <Card.Body>
              <blockquote className="blockquote mb-0">
                ...
                <footer className="blockquote-footer">
                  ...
                  <br></br> Phone:{" "}
                  <cite title="Source Title">{user.phone}</cite>
                </footer>
              </blockquote>
              <Card
                bg={"secondary"}
